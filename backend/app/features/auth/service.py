...
from app.core.store import k_lock, k_token, store
...
def login(session, req, *, ip: str = "", ua: str = "") -> TokenOut:
    ...
    if not user or not crypto.verify_password(req.password, user.password_hash):
        ...
        write_login(session, username=req.username, ok=False, ip=ip, ua=ua,
                    detail=f"密码错误({n}/{settings.MAX_FAILED})")
        session.commit()
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "用户名或密码错误")

    # 4) 成功：清零失败计数，发 JWT，写白名单
...
def me_full(session, user) -> UserOut:
    ...
    return UserOut(
        id=row.id, username=row.username, roles=row.roles,
