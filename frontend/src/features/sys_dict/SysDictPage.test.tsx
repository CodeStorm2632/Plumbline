...
describe("SysDictPage 七态", () => {
  it("无权限：提示且隐藏写操作", () => {
    render(<SysDictPage roles={["审计员"]} />);
    expect(screen.getByRole("alert")).toBeInTheDocument();
    expect(screen.queryByText("新建字典类型")).toBeNull();
  });

  it("空态", () => {
    (useDicts as ReturnType<typeof vi.fn>).mockReturnValue({ data: [], isLoading: false, isError: false });
    render(<SysDictPage roles={["管理员"]} />);
    expect(screen.getByText("暂无字典类型。")).toBeInTheDocument();
  });

  it("错误态可重试", () => {
    (useDicts as ReturnType<typeof vi.fn>).mockReturnValue({
  ...
  it("渲染类型并提示选择字典类型", () => {
    ...
    (useDictItems as ReturnType<typeof vi.fn>).mockReturnValue({ data: undefined, isLoading: false, isError: false });
    render(<SysDictPage roles={["管理员"]} />);
