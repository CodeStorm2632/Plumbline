...
LOCK_LINE_RE = re.compile(r"^\s*((?:FR|NFR)-[0-9.]+)\s*:\s*\{(.*)\}\s*$")
...
def load_lock(path):
    ...
    for ln in read(path).splitlines():
        ...
        since = (
            (SINCE_RE.search(body) or [None, "v?"])[1]
            if SINCE_RE.search(body)
            else "v?"
        )
...
def write_lock(path, lock):
    lines = [
        "# requirements-lock.yaml — append-only ID 登记（由 requirements_lock.py 维护）"
    ]
...
def merge_defs(prds):
    """合并多份 PRD 的 {id: summary}（同 id 取最长摘要）与 {id: version}。"""
    defs, vers = {}, {}
    for p in prds:
        text = read(p)
        ver = prd_version(text)
        for rid, summary in prd_defs(text).items():
            if len(summary) > len(defs.get(rid, "")):
                defs[rid] = summary
            vers.setdefault(rid, ver)  # 首次出现的 PRD version 即该 id 的 since
    return defs, vers


def cmd_gen(args):
    defs, vers = merge_defs(args.prd)
    lock = load_lock(args.lock)
    added, updated = [], []
    for rid, summary in defs.items():
        if rid not in lock:
            lock[rid] = {"since": vers.get(rid, "v?"), "summary": summary}
            added.append(rid)
        else:
    ...
    print(f"lock 写入: {args.lock}  (PRD: {', '.join(args.prd)})")
    print(
        f"  新登记 {len(added)} 条"
        + (f": {', '.join(sorted(added, key=fr_key))}" if added else "")
    )
    if updated:
...
def cmd_check(args):
        ...
        for rid in removed:
            ...
            for nid in added:
                r = difflib.SequenceMatcher(
                    None, lock[rid]["summary"], defs[nid]
                ).ratio()
                if r > score:
                    best, score = nid, r
    ...
    if removed:
        print(
            "ALARM ✗ 存在被删/回收 ID。若是有意废弃，请保留旧号并标注废弃，不要回收。"
        )
        return 1
    msg = "PASS ✓ 无 ID 被删/回收"
    if added:
...
def main(argv):
    ...
    g.add_argument("prd", nargs="+", help="一份或多份 PRD（多份取并集）")
    ...
    c.add_argument("prd", nargs="+", help="一份或多份 PRD（多份取并集）")
