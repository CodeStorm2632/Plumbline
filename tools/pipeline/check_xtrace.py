...
用法：
    python check_xtrace.py <openapi.json> <prd.md> [prd2.md ...]

多份 PRD 时合法 ID 全集取并集（共享 openapi 含多 PRD 的 x-trace 时必需）。
...
def main(argv: list[str]) -> int:
    if len(argv) < 3:
        print("用法：check_xtrace.py <openapi.json> <prd.md> [prd2.md ...]")
        return 2
    ...
    if dangling:
        ...
        return 1
    print(
        "---\nPASS ✓ 所有 x-trace 均可追溯到 PRD"
        + ("（但有接口缺 x-trace）" if missing_xtrace else "")
    )
    return 0
